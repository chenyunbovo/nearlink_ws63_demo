...
static uint16_t blufi_server_set_adv_data(uint8_t *set_adv_data, uint8_t adv_data_max_len)
{
    uint8_t idx = 0;
...
uint8_t blufi_set_adv_data(void)
{
    ...
    /* set adv data */
    uint16_t adv_data_len = blufi_server_set_adv_data(set_adv_data, EXT_ADV_OR_SCAN_RSP_DATA_LEN);
    ...
    osal_printk("%s blufi_set_adv_data adv_handle %d, len:%d, data:%s\n",
                blufi_ADV_LOG, BTH_GAP_BLE_ADV_HANDLE_DEFAULT, adv_data_len, set_adv_data);
...
uint8_t blufi_start_adv(void)
{
    ...
    osal_printk("%s blufi_start_adv adv_id %d\n", blufi_ADV_LOG, adv_id);
